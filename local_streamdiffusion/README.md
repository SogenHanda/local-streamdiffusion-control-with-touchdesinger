...
`LATENT MORPH`には入力全体の動きに応じた割合が表示されます。入力映像の局所的な動きでも履歴の影響を弱めます。大きな変化でもStreamDiffusion内部バッファを自動的にゼロへ戻しません。

### 形の変化を滑らかにつなぐモーフ（2026-09-18更新）

生成latentを短い時間幅の重み付き平均でつなぎ、VAEで画像化します。以前の微小変化だけを補正する方式から、静止した入力に対する大きな生成形状の変化も段階的に移す方式へ変更しました。古い特徴の重みは滑らかに0へ近づき、時間幅を超えると破棄されます。履歴には加工前の特徴だけを保存するため、再帰的なぼけの蓄積を避けます。

TouchDesignerの既存操作をそのまま使用できます。`history_frames`は30fps換算の時間幅として扱い、実際のFPSが変わっても変形時間をおおむね維持します。

| History Frames | 時間幅 | 用途 |
|---:|---:|---|
| 4 | 約100ms | 追従を優先 |
| 6 | 約167ms | 滑らかさと追従の中間 |
| 8 | 約233ms | ゆっくり滑らかな変形 |

まず `Latent Morph=0.8〜1.0 / History Frames=6〜8 / Input Feedback=0` を使います。時間幅はそのまま表示遅延を表す値ではありません。大きく動いた領域は、対応する入力映像の差から履歴の影響を弱め、残像を抑えます。2-stepでは出力が入力より1フレーム遅れるため、この判定にも1フレーム前の入力を使用します。

Output SmoothingはRGBの補助平滑化です。100%でも過去画素の割合を最大50%に制限し、常に現在フレームを残します。以前の100%付近で前フレームへ置き換わる挙動を防ぎます。既存の設定ファイルやOSC address、TensorRTエンジンはそのまま使えます。

この処理は光学フローによる位置合わせや中間フレーム生成ではありません。高速な動作、遮蔽、構図の大きな変化では追従や輪郭を確認してください。滑らかさを強めるほど反応が遅くなり、変形途中の細部は柔らかく見える場合があります。
